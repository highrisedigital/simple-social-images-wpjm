 ...
 */
function ssi_wpjm_has_rankmath_custom_og_image( $post_id = 0 ) {
 ...
 * @return bool             True if the post has a custom image and false otherwise.
 */
function ssi_wpjm_has_rankmath_custom_twitter_image( $post_id = 0 ) {
 ...
 */
function ssi_wpjm_rankmath_maybe_change_open_graph_image_url( $url ) {

	// if this is not a single job or we have a custom rank math open graph image.
	if ( ! is_singular( 'job_listing' ) || ssi_wpjm_has_rankmath_custom_og_image() ) {
	...
	return ssi_wpjm_ssi_get_image_url();

}

add_filter( 'rank_math/opengraph/facebook/og_image', 'ssi_wpjm_rankmath_maybe_change_open_graph_image_url' );
add_filter( 'rank_math/opengraph/facebook/og_image_secure_url', 'ssi_wpjm_rankmath_maybe_change_open_graph_image_url' );
 ...
 * @param string $url URL of the image.
 */
function ssi_wpjm_rankmath_maybe_change_twitter_image_url( $url ) {

	// if this is not a single job or we have a custom rank math twitter image.
	if ( ! is_singular() || ssi_wpjm_has_rankmath_custom_twitter_image() ) {
...
add_filter( 'rank_math/opengraph/twitter/image', 'ssi_wpjm_rankmath_maybe_change_twitter_image_url' );
